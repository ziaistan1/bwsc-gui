...
while run_server:
    ...
    <voice name="/v2/en_speaker_3">Oh my God [laughs] he is still celebrating</voice>
        ...
        try:
            barkgui.queue().launch(share=True, inbrowser=autolaunch, server_name=server_name, server_port=server_port, share=settings.server_share, prevent_thread_lock=True)
        except:
            restart_server = True
            run_server = False
        try:
